...
import { Eye, EyeOff, Shirt, X } from 'lucide-react';
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-neutral-50 px-4">
        <div className="bg-white rounded-2xl shadow-lg p-10 text-center max-w-sm w-full">
          <div className="h-16 w-16 bg-accent-50 rounded-full flex items-center justify-center mx-auto mb-5">
            <Shirt className="h-8 w-8 text-accent-700" />
          </div>
          ...
          <button
            ...
            className="w-full bg-accent-700 text-white py-3 rounded-full font-bold hover:bg-accent-800 transition-colors"
          >
            Back to Sign In
          </button>
        </div>
  ...
  return (
    <div className="min-h-screen flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 relative bg-neutral-900">
        <Image
          src="https://images.unsplash.com/photo-1627123424574-724758594e93?w=1200&q=85"
          alt="Ethiopian leather craft"
          fill
          ...
          unoptimized
        />
        <div className="absolute inset-0 bg-gradient-to-tr from-neutral-900/80 to-transparent" />
        <div className="relative z-10 p-12 flex flex-col justify-end">
          ...
          <h2 className="text-4xl font-serif font-bold text-white mb-4">
            ...
            <span className="text-accent-400 font-light italic">Ethically Sourced.</span>
          </h2>
          <p className="text-neutral-300 text-lg max-w-sm">
            Create an account to track your orders, save your details for faster checkout, and manage your preferences.
          </p>
        </div>
      </div>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center p-6 bg-white">
        <div className="w-full max-w-md">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {!isLogin && (
              <div>
                ...
                <input
                  ...
                  className="w-full border border-neutral-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-accent-500 transition"
                />
              </div>
            )}
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                ...
                {isLogin && (
                  <button type="button" onClick={() => setShowResetModal(true)} className="text-xs text-accent-600 hover:underline">
                    Forgot password?
                  </button>
                )}
              </div>
              <div className="relative">
                <input
                  ...
                  required
                  className="w-full border border-neutral-200 rounded-xl px-4 py-3 pr-11 text-sm focus:outline-none focus:ring-2 focus:ring-accent-500 transition"
                />
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-accent-700 text-white py-3 rounded-full font-bold text-sm hover:bg-accent-800 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
          ...
          <p className="text-center text-xs text-neutral-400 mt-6">
            ...
            <button onClick={() => { setIsLogin(!isLogin); setError(''); }} className="text-accent-600 font-semibold hover:underline">
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6 animate-fadeIn">
            ...
            <form onSubmit={handlePasswordReset} className="space-y-4">
              <input
                ...
                required
                className="w-full border border-neutral-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-accent-500"
              />
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-accent-700 text-white py-3 rounded-full font-bold text-sm hover:bg-accent-800 transition-colors disabled:opacity-50"
              >
                Send Reset Link
              </button>
            </form>
